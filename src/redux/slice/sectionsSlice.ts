...
export const sectionSlice = createSlice({
  ...
  reducers: {
    ...
    addQuestion(state, action: PayloadAction<string>) {
      state.newSection.questions.unshift({
        questionText: action.payload,
        id: state.newSection.questions.length,
      });
    },
    ...
    removeQuestion(state, action: PayloadAction<{ id: number }>) {
      state.newSection.questions = state.newSection.questions.filter(
    ...
    updateSection(state, action: PayloadAction<SectionInterface>) {
      const sectionIndex = state.doneSections.findIndex(
